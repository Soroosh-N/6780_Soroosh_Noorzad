...
void log_data(void) {
    ...
    char str[80];
    snprintf(str, sizeof(str), "RPM: %d, Target: %d, Duty: %d, K_I: %d, K_P: %d\r\n", motor_speed, target_rpm, duty_cycle, Ki, Kp);
...
void PI_update(void) {
     ...
     */
    // 3. Clamp the integral to prevent wind-up (recommended 0 to 3200)
    ...
    */
    ...
    // 5. Scale output from fixed-point to duty cycle (divide by 32 = >>5)
    output = output >> 5;
     
    /// TODO: Clamp the output value between 0 and 100
    // 6. Clamp output between 0 and 100
    if (output > 100)
        output = 100;
    else if (output < 0)
        output = 0;
